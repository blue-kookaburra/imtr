...
1. **Planned-works scraper (primary — no API key needed).** GitHub Actions cron every 2 days runs `npm run scrape`, which fetches transport.vic.gov.au planned-works pages **via curl** (the site's bot protection fingerprints TLS and blocks node fetch / serverless runtimes; curl passes) and commits `data/disruptions.json`, which the app bundles. Data lives in each page's embedded `__NEXT_DATA__` JSON as HTML tables ("This week" + "Four-week forecast" weekly accordions). Row formats handled: `from X to Y`, `between X and Y`, `between X, Y and Z` (multi-branch, spanned per line), sectionless `Buses replace trains.` (whole line), `X, Y and Z passengers` line lists, City Loop station aliases, and `evening trains` (assumed from 6pm). The index page `/plan-a-journey/planned-works` is scraped for per-line page URLs so new pages are picked up automatically.
